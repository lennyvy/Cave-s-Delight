# Cave's Delight 🍄

**Cave's Delight** is an expansion mod for **NeoForge 1.21.1** that enhances cave life with culinary adventures – inspired by *Farmer's Delight*. The mod adds new edible plants, mushrooms, and animal products, along with special cave-themed recipes for the cooking pot.

---

## 🌟 Features

### 🌱 New Plants & Blocks
- **Chess Flower** – A rare cave flower whose petals can be processed into a nourishing soup.
- **Shimmering Mushroom** – A luminous mushroom that grows in dark caves and serves as an ingredient for special dishes.
- **Small Shimmering Mushrooms** – A smaller variant of the Shimmering Mushroom that spawns in clusters.

### 🍲 New Food Items & Materials
- **Amethyst Powder** – A fine powder made from amethyst shards, used as an ingredient in brewing and cooking recipes.
- **Chess Flower Petals** – The petals of the Chess Flower, essential for making Chess Flower Soup.
- **Chess Flower Soup** – A hearty soup prepared in the cooking pot.
- **Axolotl Products**:
  - *Raw Axolotl Ham* – Raw axolotl ham.
  - *Cooked Axolotl Ham* – Cooked axolotl ham.
  - *Cooked Axolotl Slice* – Sliced, cooked axolotl meat.
- **Cooked Glow Lichen** – A cooked version of glow lichen, served as a snack or used as an ingredient.

### 🍳 Cooking Pot Recipes
The mod expands the cooking pot system with new recipes, including:
- Preparing Axolotl Ham in the cooking pot.
- Cooking Chess Flower Soup.
- Additional processing recipes using Amethyst Powder and other cave ingredients.

### 🧪 Brewing & Processing
- Special brewing recipes using Amethyst Powder and other cave ingredients.
- Reverse-crafting recipes that break items back down into their base components.

### 🗺️ World Generation
- New plants and mushrooms are automatically generated in suitable cave biomes (managed via biome modifiers).

### 🔧 Compatibility
- **JEI Integration** – All recipes are displayed in the Just Enough Items menu.
- **Farmer's Delight** – The mod is designed as an add-on and hooks into its cooking pot system (via mixins).

---

## 📦 Installation

1. Install **NeoForge 1.21.1**.
2. Download **Farmer's Delight** (for NeoForge 1.21.1) and place it in your `mods` folder.
3. Copy the **Cave's Delight** JAR file into the `mods` folder.
4. Launch Minecraft.

---

## 🛠️ Development

- **Mod Loader**: NeoForge 1.21.1
- **Language**: Java
- **Dependencies**: Farmer's Delight and optional JEI 

---

## 📝 Future Plans

- More cave plants and ressources
- Additional cooking pot recipes with unique effects
- Building blocks centered around die salt item. 

---

## 🤝 Contributing

Have ideas or want to report a bug? Feel free to msg me on discord @ryfle_

---

## 📄 License

This project is licensed under the [MIT License](LICENSE).

---

*Happy exploring and cooking in the depths of Minecraft!* 🍲🕯️
